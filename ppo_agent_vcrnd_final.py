...
class PpoAgent(object):
    ...
    def __init__(self, *, scope,
        ...
        self.vc_rnd = bool(vc_rnd)
        self.vc_rnd_mode = vc_rnd_mode
        self.vc_rnd_lambda = float(vc_rnd_lambda)
        self.vc_rnd_beta = float(vc_rnd_beta)
        self.vc_rnd_min_scale = float(vc_rnd_min_scale)
        ...
        self.vc_rnd_running_value_mean = 0.0
        self.vc_rnd_eps = 1e-8

    def _apply_value_conditioning(self, rews_int_norm):
        """Scale normalized intrinsic rewards using the extrinsic value head.

        Proposal mapping:
        1) compute running mean of V_ext(s_t) with EMA beta
        2) compute Delta V_t = |V_ext(s_t) - Vbar_t|
        3) scale normalized intrinsic reward with inverse or sigmoid gating
        """
        if not self.vc_rnd:
            return rews_int_norm, np.ones_like(rews_int_norm), np.zeros_like(rews_int_norm), self.vc_rnd_running_value_mean

        values = self.I.buf_vpreds_ext.astype(np.float32)
        running_mean = float(self.vc_rnd_running_value_mean)

        # Compute the EMA in time-major order so the running mean evolves with the rollout.
        # This avoids leaking env0's last step directly into env1's first step via row-major flattening.
        values_time_major = values.T  # [nsteps, nenvs]
        value_mean_trace_time_major = np.zeros_like(values_time_major, dtype=np.float32)
        for t in range(values_time_major.shape[0]):
            step_values = values_time_major[t]
            running_mean = (1.0 - self.vc_rnd_beta) * running_mean + self.vc_rnd_beta * float(np.mean(step_values))
            value_mean_trace_time_major[t, :] = running_mean
        value_mean_trace = value_mean_trace_time_major.T
        delta_v = np.abs(values - value_mean_trace)

        if self.vc_rnd_mode == 'inverse':
            scale = 1.0 / (1.0 + self.vc_rnd_lambda * delta_v)
        elif self.vc_rnd_mode == 'sigmoid':
            # Proposal-faithful version:
            #   g(V(s_t)) = sigma(-lambda * DeltaV_t)
            # Since DeltaV_t >= 0, this stays in (0, 0.5].
            scale = 1.0 / (1.0 + np.exp(self.vc_rnd_lambda * delta_v))

            # Behaviorally safer alternative that reaches 1.0 when DeltaV_t == 0.
            # Keep commented so it can replace the proposal-faithful version directly.
            # scale = 2.0 / (1.0 + np.exp(self.vc_rnd_lambda * delta_v))
            # scale = np.clip(scale, 0.0, 1.0)

            # Mathematically better-aligned alternative for your stated motivation:
            # only downweight states whose value falls below the running baseline.
            # delta_v_low = np.maximum(0.0, value_mean_trace - values)
            # scale = 1.0 / (1.0 + self.vc_rnd_lambda * delta_v_low)
            # delta_v = delta_v_low
        else:
            raise ValueError('Unknown vc_rnd_mode: {}'.format(self.vc_rnd_mode))

        if self.vc_rnd_min_scale > 0.0:
            scale = np.maximum(scale, self.vc_rnd_min_scale)

        scale = scale.astype(np.float32)
        delta_v = delta_v.astype(np.float32)
        self.vc_rnd_running_value_mean = float(running_mean)
        return rews_int_norm * scale, scale, delta_v, float(running_mean)
    ...
    def update(self):
        ...
        local_eprews = list(self.I.statlists["eprew"])
        eprews = MPI.COMM_WORLD.allgather(np.mean(local_eprews) if len(local_eprews) > 0 else np.nan)
        ...
        rews_int_norm = self.I.buf_rews_int / np.sqrt(self.I.rff_rms_int.var + self.vc_rnd_eps)
        # Exact proposal-style normalization with mean subtraction, kept commented out for easy swap.
        # rews_int_norm = (self.I.buf_rews_int - self.I.rff_rms_int.mean) / np.sqrt(self.I.rff_rms_int.var + self.vc_rnd_eps)
        rews_int, vc_scale, vc_delta_v, vc_value_mean = self._apply_value_conditioning(rews_int_norm)
        ...
        info = dict(
            ...
            vc_rnd_enabled = float(self.vc_rnd),
            vc_rnd_scale_mean = float(np.mean(vc_scale)),
            vc_rnd_scale_min = float(np.min(vc_scale)),
            vc_rnd_scale_max = float(np.max(vc_scale)),
            vc_rnd_delta_v_mean = float(np.mean(vc_delta_v)),
            vc_rnd_running_value_mean = float(vc_value_mean),
        ...
        to_record = {'acs': self.I.buf_acs,
                     ...
                     'rews_int_vcrnd': rews_int,
                     'vc_rnd_scale': vc_scale,
                     'vc_rnd_delta_v': vc_delta_v,
                     'rews_ext': self.I.buf_rews_ext,
    ...
    def step(self):
        ...
        for l in range(self.I.nlump):
            ...
            for env_pos_in_lump, info in enumerate(infos):
                if 'episode' in info:
                    ...
                    info_with_places['places'] = info['episode'].get('visited_rooms', [])
        ...
        if t == self.nsteps - 1 and not self.disable_policy_update:
            ...
            global_deque_mean = dict_gather(self.comm_log, {n: (np.mean(dvs) if len(dvs) > 0 else np.nan) for n, dvs in self.I.statlists.items()}, op='mean')
